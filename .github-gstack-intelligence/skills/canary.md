...
> Source: `garrytan/gstack` @ `7ea6ead9fa88` from `canary/SKILL.md.tmpl`.
...
- This is the extracted `/canary` skill prompt committed into the repository at refresh time.
...
```bash
eval "$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" 2>/dev/null || echo "SLUG=unknown")"
mkdir -p .github-gstack-intelligence/state/local/canary-reports
mkdir -p .github-gstack-intelligence/state/local/canary-reports/baselines
mkdir -p .github-gstack-intelligence/state/local/canary-reports/screenshots
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".github-gstack-intelligence/state/local/canary-reports/baselines/<page-name>.png"
$B console --errors
$B perf
$B text
```
...
Save the baseline manifest to `.github-gstack-intelligence/state/local/canary-reports/baseline.json`:
...
Extract the top 5 internal navigation links from the `links` output. Always include the homepage. Present the page list via GitHub follow-up comment:
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".github-gstack-intelligence/state/local/canary-reports/screenshots/pre-<page-name>.png"
$B console --errors
$B perf
```
...
```bash
$B goto <page-url>
$B snapshot -i -a -o ".github-gstack-intelligence/state/local/canary-reports/screenshots/<page-name>-<check-number>.png"
$B console --errors
$B perf
```
...
**If a CRITICAL or HIGH alert is detected**, immediately notify the user via GitHub follow-up comment:
...
Save report to `.github-gstack-intelligence/state/local/canary-reports/{date}-canary.md` and `.github-gstack-intelligence/state/local/canary-reports/{date}-canary.json`.
