...
> Source: `garrytan/gstack` @ ref `main` from `autoplan/SKILL.md.tmpl`.
...
- This is the extracted `/autoplan` skill prompt committed into the repository at refresh time.
...
feasibility blocker (not a preference), the GitHub follow-up comment framing explicitly
...
GitHub follow-up comment: you do, using the 6 principles, instead of the user.
...
echo "RESTORE_PATH=$HOME/.github-gstack-intelligence/state/local/projects/$SLUG/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t .github-gstack-intelligence/state/results/$SLUG/*-design-*.md 2>/dev/null | head -1`
...
- **GATE: Present premises to user for confirmation** — this is the ONE GitHub follow-up comment
...
- Test plan: generate artifact at `.github-gstack-intelligence/state/results/$SLUG/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at .github-gstack-intelligence/state/results/$SLUG/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
<!-- CI-ADAPTED: {{TASKS_SECTION_AGGREGATE}} expansion is omitted. Implement the GitHub-native replacement in the lifecycle layer when this skill is activated. -->

**STOP here and present the final state to the user.**

Present as a message, then use GitHub follow-up comment:
...
GitHub follow-up comment options:
...
.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```
...
.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
- **Two gates.** The non-auto-decided GitHub follow-up comments are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
