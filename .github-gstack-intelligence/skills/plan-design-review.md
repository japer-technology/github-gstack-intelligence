...
> Source: `garrytan/gstack` @ `6169273d16b7` from `plan-design-review/SKILL.md.tmpl`.
...
- This is the extracted `/plan-design-review` skill prompt committed into the repository at refresh time.
...
### 0D. Focus Areas
GitHub follow-up comment: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
`.github-gstack-intelligence/state/results/$SLUG/designs/` (user config directory, not project files).
...
- `mkdir -p .github-gstack-intelligence/state/results/$SLUG/designs/...`
...
Generate design variants sequentially in CI mode. Present comparison boards for human review through GitHub comments or issue attachments.

**Do NOT use GitHub follow-up comment to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use GitHub follow-up comment to confirm you understood the feedback correctly, never to re-ask what they chose.
...
Cross-reference design findings against industry standards and the project's stated design system. Surface conflicting recommendations for user resolution.
...
5. GitHub follow-up comment if there's a genuine design choice to resolve
...
**STOP.** GitHub follow-up comment once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** GitHub follow-up comment once per issue. Do NOT batch. Recommend + WHY.
...
AI slop blacklist: reject generic card grids, centered hero sections with stock photos, 3-column feature layouts, gradient buttons with drop shadows, and any pattern that looks like every other AI-generated site.
...
Each decision = one GitHub follow-up comment with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
GitHub follow-up comment: "The review passes changed [list major design changes]. Want me to regenerate mockups to reflect the updated plan? This ensures the visual reference matches what we're actually building."
...
Follow the GitHub follow-up comment format from the Preamble above. Additional rules for plan design reviews:
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual GitHub follow-up comment. Never batch TODOs — one per question. Never silently skip this step.
...
If any GitHub follow-up comment goes unanswered, note it here. Never silently default to an option.
...
|----------------|-------------|-----------|-------|
| [screen name]  | .github-gstack-intelligence/state/results/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
