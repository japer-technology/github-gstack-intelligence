...
> Source: `garrytan/gstack` @ `a1a933614cda` from `review/SKILL.md.tmpl`.
...
- This is the extracted `/review` skill prompt committed into the repository at refresh time.
...
If planning artifacts are unavailable, treat the issue body, pull request body, and commit history as the stated intent and continue.
...
Read `.github-gstack-intelligence/skills/references/review-checklist.md`.
...
Search repository-local state and issue context first before making review recommendations.
...
Use the same confidence bar, but report through GitHub comments and persisted state files rather than local CLI prompts.
...
If there are ASK items remaining, present them in ONE GitHub follow-up comment:
...
If 3 or fewer ASK items, you may use individual GitHub follow-up comments instead of batching.
...
2. **FALSE POSITIVE comments:** Present each one via GitHub follow-up comment:
...
3. **VALID BUT ALREADY FIXED comments:** Reply using the **Already Fixed reply template** from greptile-triage.md — no GitHub follow-up comment needed:
...
```bash
.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"commit":"COMMIT"}'
```
...
Persist durable review outcomes in `.github-gstack-intelligence/state/results/review/` when the lifecycle layer is ready to store them.
