...
> Source: `garrytan/gstack` @ ref `main` from `design-consultation/SKILL.md.tmpl`.
...
- This is the extracted `/design-consultation` skill prompt committed into the repository at refresh time.
...
**GitHub follow-up comment Q1 — include ALL of these:**
...
**GitHub follow-up comment Q2 — present the full proposal with SAFE/RISK breakdown:**
...
Each drill-down is one focused GitHub follow-up comment. After the user decides, re-check coherence with the rest of the system.
...
_DESIGN_DIR="$HOME/.github-gstack-intelligence/state/local/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
**GitHub follow-up comment Q-final — show summary and confirm:**
