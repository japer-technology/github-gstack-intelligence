...
> Source: `garrytan/gstack` @ `7ea6ead9fa88` from `design-review/SKILL.md.tmpl`.
...
- This is the extracted `/design-review` skill prompt committed into the repository at refresh time.
...
REPORT_DIR=.github-gstack-intelligence/state/results/$SLUG/designs/design-audit-$(date +%Y%m%d)
...
```
.github-gstack-intelligence/state/results/$SLUG/designs/design-audit-{YYYYMMDD}/
...
```
Write a one-line summary to `.github-gstack-intelligence/state/results/{slug}/{user}-{branch}-design-audit-{datetime}.md` with a pointer to the full report in `$REPORT_DIR`.
