...
> Source: `garrytan/gstack` @ `7ea6ead9fa88` from `investigate/SKILL.md.tmpl`.
...
- This is the extracted `/investigate` skill prompt committed into the repository at refresh time.
...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via GitHub follow-up comment.
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use GitHub follow-up comment:
   ```
...
5. **If the fix touches >5 files:** Use GitHub follow-up comment to flag the blast radius:
   ```
...
- **If fix touches >5 files → GitHub follow-up comment** about blast radius before proceeding.
- **Completion status:**
