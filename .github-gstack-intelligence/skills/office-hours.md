...
gbrain:
  ...
  context_queries:
    ...
    - id: builder-profile
      kind: filesystem
      glob: ".github-gstack-intelligence/state/builder-profile.jsonl"
      tail: 1
...
> Source: `garrytan/gstack` @ ref `main` from `office-hours/SKILL.md.tmpl`.
...
- This is the extracted `/office-hours` skill prompt committed into the repository at refresh time.
...
4. **List existing design docs for this project:**
   ...
   ls -t .github-gstack-intelligence/state/results/$SLUG/*-design-*.md 2>/dev/null
   ```
...
5. **Ask: what's your goal with this?** This is a real question, not a formality. The answer determines everything about how the session runs.
   ...
   Via GitHub follow-up comment, ask:
...
Ask these questions **ONE AT A TIME** via GitHub follow-up comment. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via GitHub follow-up comment. The goal is to brainstorm and sharpen the idea, not interrogate.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" .github-gstack-intelligence/state/results/$SLUG/*-design-*.md 2>/dev/null
```

If matches found, read the matching design docs and surface them:
- "FYI: Related design found — '{title}' by {user} on {date} (branch: {branch}). Key overlap: {1-line summary of relevant section}."
- Ask via GitHub follow-up comment: "Should we build on this prior design or start fresh?"

This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `.github-gstack-intelligence/state/results/`.
...
**Privacy gate:** Before searching, use GitHub follow-up comment: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Emit ONE GitHub follow-up comment that lists every alternative (A/B and optionally C) as numbered options, using the preamble's GitHub follow-up comment Format section. The GitHub follow-up comment call is a tool_use, not prose — write the question text and call the tool.
...
Design mockup generation via local tools is not available in CI mode. If a mockup would be valuable, describe the mockup in detail (layout, components, interactions, responsive behavior) in the GitHub comment so the user can visualize or create it. Reference DESIGN.md if it exists for design system constraints.

Quick sketch generation via local tools is not available in CI mode. If a sketch would clarify an approach, describe it as a text-based wireframe using ASCII art or a structured layout description in the GitHub comment. Focus on information hierarchy and user flow, not visual polish.
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t .github-gstack-intelligence/state/results/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `.github-gstack-intelligence/state/results/{slug}/{user}-{branch}-design-{datetime}.md`.
...
Present the reviewed design doc to the user via GitHub follow-up comment:
...
```bash
PROFILE=$(.github-gstack-intelligence/skills/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
Then use GitHub follow-up comment: "Would you consider applying to Y Combinator?"
...
**Builder Journey Summary** (session 5+): Auto-generate `.github-gstack-intelligence/state/builder-journey.md`
...
Auto-generate updated `.github-gstack-intelligence/state/builder-journey.md` with narrative arc. Open it.
...
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> .github-gstack-intelligence/state/analytics/skill-usage.jsonl 2>/dev/null || true
```

3. Use GitHub follow-up comment to offer opening the resources:
...
The design doc at `.github-gstack-intelligence/state/results/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one GitHub follow-up comment.
