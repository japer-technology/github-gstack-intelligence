...
> Source: `garrytan/gstack` @ `cf73db5f1904` from `office-hours/SKILL.md.tmpl`.
...
<!-- CI-ADAPTED: {{CODEX_SECOND_OPINION}} expansion is omitted. Implement the GitHub-native replacement in the lifecycle layer when this skill is activated. -->
...
Present via GitHub follow-up comment. Do NOT proceed without user approval of the approach.

---

<!-- CI-ADAPTED: {{DESIGN_MOCKUP}} expansion is omitted. Implement the GitHub-native replacement in the lifecycle layer when this skill is activated. -->

<!-- CI-ADAPTED: {{DESIGN_SKETCH}} expansion is omitted. Implement the GitHub-native replacement in the lifecycle layer when this skill is activated. -->
...
```bash
eval "$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" 2>/dev/null)" 2>/dev/null || true
