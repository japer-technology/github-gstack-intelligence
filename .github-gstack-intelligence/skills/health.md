...
> Source: `garrytan/gstack` @ ref `main` from `health/SKILL.md.tmpl`.
...
- This is the extracted `/health` skill prompt committed into the repository at refresh time.
...
After auto-detection, present the detected tools via GitHub follow-up comment:
...
queue_component:   10 if .github-gstack-intelligence/state/.brain-queue.jsonl has <10 lines;
                    7 if 10-100; 0 if >=100 (suggests secret-scan rejections
                    piling up). N/A if artifacts_sync_mode == off.
push_component:    10 if (now - mtime of .github-gstack-intelligence/state/.brain-last-push) < 24h;
...
Append one JSONL line to `.github-gstack-intelligence/state/results/$SLUG/health-history.jsonl`:
...
Read the last 10 entries from `.github-gstack-intelligence/state/results/$SLUG/health-history.jsonl` (if the
file exists and has prior entries).

```bash
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p .github-gstack-intelligence/state/results
tail -10 .github-gstack-intelligence/state/results/$SLUG/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
