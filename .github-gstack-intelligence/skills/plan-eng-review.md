...
> Source: `garrytan/gstack` @ ref `main` from `plan-eng-review/SKILL.md.tmpl`.
...
- This is the extracted `/plan-eng-review` skill prompt committed into the repository at refresh time.
...
If the complexity check triggers (8+ files or 2+ new classes/services), STOP before any review-section work. Call GitHub follow-up comment: name what's overbuilt, propose a minimal version that achieves the core goal, ask whether to reduce or proceed as-is. The GitHub follow-up comment call is a tool_use, not prose — call the tool directly.

**STOP.** Do NOT proceed to Section 1 (Architecture review), edit the plan file with a proposed scope reduction, or call ExitPlanMode until the user responds. Naming the 80% solution in chat prose and continuing — or loading the GitHub follow-up comment schema via ToolSearch and then never invoking it — is the failure mode this gate exists to prevent.
...
For each issue found in this section, call GitHub follow-up comment individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one GitHub follow-up comment. Use the preamble's GitHub follow-up comment Format section. The GitHub follow-up comment call is a tool_use, not prose — call the tool directly.

**STOP.** Do NOT proceed to the next review section, edit the plan file with the proposed fix, or call ExitPlanMode until the user responds. An issue with an "obvious fix" is still an issue and still needs explicit user approval before it lands in the plan. Loading the GitHub follow-up comment schema via ToolSearch and then writing the recommendation as chat prose is the failure mode this gate exists to prevent.
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual GitHub follow-up comment. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.claude/skills/review/TODOS-format.md`.
...
```bash
.github-gstack-intelligence/state/results/review/review-log.json '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
...
Use GitHub follow-up comment with only the applicable options:
- **A)** Run /plan-design-review (only if UI scope detected and no design review exists)
- **B)** Run /plan-ceo-review (only if significant product change and no CEO review exists)
- **C)** Ready to implement — run /ship when done

## Unresolved decisions
If the user does not respond to an GitHub follow-up comment or interrupts to move on, note which decisions were left unresolved. At the end of the review, list these as "Unresolved decisions that may bite you later" — never silently default to an option.
