...
- This is the extracted `/learn` skill prompt committed into the repository at refresh time.
...
.github-gstack-intelligence/skills/bin/gstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" 2>/dev/null)"
.github-gstack-intelligence/skills/bin/gstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" 2>/dev/null)"
.github-gstack-intelligence/skills/bin/gstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)" 2>/dev/null)"
.github-gstack-intelligence/skills/bin/gstack-learnings-search --limit 50 2>/dev/null
```
