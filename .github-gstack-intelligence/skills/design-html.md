...
> Source: `garrytan/gstack` @ ref `main` from `design-html/SKILL.md.tmpl`.
...
- This is the extracted `/design-html` skill prompt committed into the repository at refresh time.
...
_CEO=$(ls -t .github-gstack-intelligence/state/results/$SLUG/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t .github-gstack-intelligence/state/results/$SLUG/designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t .github-gstack-intelligence/state/results/$SLUG/designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t .github-gstack-intelligence/state/results/$SLUG/designs/*/finalized.html 2>/dev/null | head -1)
...
Then check for prior finalized.html. If `FINALIZED` was also found, use GitHub follow-up comment:
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** use GitHub follow-up comment to gather what the user wants to build. Ask about:
...
If a framework is detected, use GitHub follow-up comment:
...
[ -z "$_PRETEXT_VENDOR" ] && [ -f .github-gstack-intelligence/skills/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=.github-gstack-intelligence/skills/design-html/vendor/pretext.js
...
`.github-gstack-intelligence/state/results/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`.github-gstack-intelligence/state/results/$SLUG/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
LOOP:
  ...
  3. GitHub follow-up comment (adjust wording based on mode):
...
Maximum 10 iterations. If the user hasn't said "done" after 10, use GitHub follow-up comment:
