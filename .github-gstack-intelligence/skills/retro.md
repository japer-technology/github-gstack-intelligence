...
gbrain:
  ...
  context_queries:
    - id: prior-retros
      kind: filesystem
      glob: ".github-gstack-intelligence/state/results/{repo_slug}/retros/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior retros for this project"
    - id: recent-timeline
      kind: filesystem
      glob: ".github-gstack-intelligence/state/results/{repo_slug}/timeline.jsonl"
      tail: 30
...
> Source: `garrytan/gstack` @ ref `main` from `retro/SKILL.md.tmpl`.
...
- This is the extracted `/retro` skill prompt committed into the repository at refresh time.
...
```bash
[ -f .github-gstack-intelligence/state/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `.github-gstack-intelligence/state/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
if [ -z "$_RETRO_GUARD_VERDICT" ]; then
  ...
  if [ -n "$_RETRO_LATEST_ISO" ]; then
    ...
    # ask the user via GitHub follow-up comment rather than proceeding.
...
cat .github-gstack-intelligence/state/greptile-history.md 2>/dev/null || true
...
cat .github-gstack-intelligence/state/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `.github-gstack-intelligence/state/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `.github-gstack-intelligence/state/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `.github-gstack-intelligence/state/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
cat .github-gstack-intelligence/state/results/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
```bash
DISCOVER_BIN=""
[ -x .github-gstack-intelligence/skills/bin/gstack-global-discover ] && DISCOVER_BIN=.github-gstack-intelligence/skills/bin/gstack-global-discover
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t .github-gstack-intelligence/state/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p .github-gstack-intelligence/state/retros
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls .github-gstack-intelligence/state/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `.github-gstack-intelligence/state/retros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `.github-gstack-intelligence/state/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
