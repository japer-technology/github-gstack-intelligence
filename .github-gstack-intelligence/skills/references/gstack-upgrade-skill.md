...
[ -z "$_AUTO" ] && _AUTO=$(.github-gstack-intelligence/config.json get auto_upgrade 2>/dev/null || true)
echo "AUTO_UPGRADE=$_AUTO"
```

**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip GitHub follow-up comment. Log "Auto-upgrading gstack v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/gstack-upgrade` manually to retry."

**Otherwise**, use GitHub follow-up comment:
...
```bash
.github-gstack-intelligence/config.json set auto_upgrade true
```
Tell user: "Auto-upgrade enabled. Future updates will install automatically." Then proceed to Step 2.

**If "Not now":** Write snooze state with escalating backoff (first snooze = 24h, second = 48h, third+ = 1 week), then continue with the current skill. Do not mention the upgrade again.
```bash
_SNOOZE_FILE=.github-gstack-intelligence/state/update-snoozed
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `auto_upgrade: true` in `.github-gstack-intelligence/state/config.yaml` for automatic upgrades."

**If "Never ask again":**
```bash
.github-gstack-intelligence/config.json set update_check false
```
Tell user: "Update checks disabled. Run `.github-gstack-intelligence/config.json set update_check true` to re-enable."
...
elif [ -d "$HOME/.github-gstack-intelligence/state/local/repos/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.github-gstack-intelligence/state/local/repos/gstack"
...
```bash
mkdir -p ~/.gstack
echo "$OLD_VERSION" > .github-gstack-intelligence/state/just-upgraded-from
rm -f .github-gstack-intelligence/state/last-update-check
rm -f .github-gstack-intelligence/state/update-snoozed
```
...
```bash
.github-gstack-intelligence/skills/bin/gstack-update-check --force 2>/dev/null || \
