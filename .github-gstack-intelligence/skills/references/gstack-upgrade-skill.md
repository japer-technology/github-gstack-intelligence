...
> Source: `garrytan/gstack` @ ref `main` from `gstack-upgrade/SKILL.md.tmpl`.
...
- This is the extracted `/gstack-upgrade` skill prompt committed into the repository at refresh time.
...
```bash
_SNOOZE_FILE="$HOME/.github-gstack-intelligence/state/local/update-snoozed"
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
_TEAM_MODE=$(.github-gstack-intelligence/config.json get team_mode 2>/dev/null || echo "false")
