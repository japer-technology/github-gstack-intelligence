...
> Source: `garrytan/gstack` @ ref `main` from `codex/SKILL.md.tmpl`.
...
(`CLAUDE_PLANS_DIR` set), a global `.github-gstack-intelligence/skills/` install, or a CI
...
user should do, in the canonical format the GitHub follow-up comment judge grades:
...
should do, in the canonical format the GitHub follow-up comment judge grades:
...
canonical format the GitHub follow-up comment judge grades:
...
- **Detect skill-file rabbit holes.** After receiving Codex output, scan for signs
  that Codex got distracted by skill files: `.github-gstack-intelligence/config.json`, `gstack-update-check`,
