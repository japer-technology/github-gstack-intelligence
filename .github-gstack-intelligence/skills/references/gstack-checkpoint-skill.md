...
> Source: `garrytan/gstack` @ ref `main` from `checkpoint/SKILL.md.tmpl`.
...
- This is the extracted `/checkpoint` skill prompt committed into the repository at refresh time.
...
SLUG=$(basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
mkdir -p .github-gstack-intelligence/state/results
CHECKPOINT_DIR="$HOME/.github-gstack-intelligence/state/local/projects/$SLUG/checkpoints"
...
After presenting the checkpoint, ask via GitHub follow-up comment:
...
- **Infer, don't interrogate.** Use git state and conversation context to fill in
  the checkpoint. Only use GitHub follow-up comment if the title genuinely cannot be inferred.
