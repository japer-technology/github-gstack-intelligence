...
> Source: `garrytan/gstack` @ `7ea6ead9fa88` from `SKILL.md.tmpl`.
...
- This is the extracted `/gstack` skill prompt committed into the repository at refresh time.
...
sessions via `.github-gstack-intelligence/config.json`.
...
If the user opts out of suggestions, run `# .github-gstack-intelligence/config.json is not available in CI mode: proactive false`.
If they opt back in, run `# .github-gstack-intelligence/config.json is not available in CI mode: proactive true`.
...
Snapshot system is replaced by Playwright's accessibility tree (`page.accessibility.snapshot()`) and screenshot capabilities in CI mode.

## Command Reference

Browse command reference is replaced by Playwright API in CI mode. Use `page.goto()`, `page.screenshot()`, `page.evaluate()`, `page.click()`, `page.fill()`, `page.locator()`, and `page.waitForSelector()` for equivalent functionality.
