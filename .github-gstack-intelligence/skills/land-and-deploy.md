...
> Source: `garrytan/gstack` @ ref `main` from `land-and-deploy/SKILL.md.tmpl`.
...
- This is the extracted `/land-and-deploy` skill prompt committed into the repository at refresh time.
...
if [ ! -f .github-gstack-intelligence/state/results/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat .github-gstack-intelligence/state/results/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
.github-gstack-intelligence/skills/bin/gstack-review-read 2>/dev/null
```
...
Present the full dry-run results to the user via GitHub follow-up comment:
...
```bash
mkdir -p .github-gstack-intelligence/state/results/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > .github-gstack-intelligence/state/results/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat .github-gstack-intelligence/skills/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
```bash
eval $(git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
4. If no deploy workflows detected and no URL provided: use GitHub follow-up comment once:
...
If deploy fails (`conclusion` is `failure`): use GitHub follow-up comment:
...
```bash
$B snapshot -i -a -o ".github-gstack-intelligence/state/local/deploy-reports/post-deploy.png"
```
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use GitHub follow-up comment:
...
```bash
mkdir -p .github-gstack-intelligence/state/local/deploy-reports
```
...
Save report to `.github-gstack-intelligence/state/local/deploy-reports/{date}-pr{number}-deploy.md`.
