...
> Source: `garrytan/gstack` @ `cf73db5f1904` from `plan-ceo-review/SKILL.md.tmpl`.
...
<!-- CI-ADAPTED: {{SPEC_REVIEW_LOOP}} expansion is omitted. Implement the GitHub-native replacement in the lifecycle layer when this skill is activated. -->
...
**STOP.** GitHub follow-up comment once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only use GitHub follow-up comment when there is a genuine decision with meaningful tradeoffs.
