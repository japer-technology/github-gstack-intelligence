...
> Source: `garrytan/gstack` @ ref `main` from `design-shotgun/SKILL.md.tmpl`.
...
- This is the extracted `/design-shotgun` skill prompt committed into the repository at refresh time.
...
_PREV=$(find .github-gstack-intelligence/state/results/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -5)
...
```bash
setopt +o nomatch 2>/dev/null || true
ls .github-gstack-intelligence/state/results/$SLUG/*office-hours* 2>/dev/null | head -5
```
...
**GitHub follow-up comment with pre-filled context:** Pre-fill what you inferred from the codebase,
...
```bash
setopt +o nomatch 2>/dev/null || true
_TASTE=$(find .github-gstack-intelligence/state/results/$SLUG/designs/ -name "approved.json" -maxdepth 2 2>/dev/null | sort -r | head -10)
```
...
Use GitHub follow-up comment to confirm before spending API credits:
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output .github-gstack-intelligence/state/...`
...
After receiving feedback (via HTTP POST or GitHub follow-up comment fallback), output a clear
...
Use GitHub follow-up comment to confirm before saving.
...
If standalone, offer next steps via GitHub follow-up comment:
...
1. **Never save to `.context/`, `docs/designs/`, or `/tmp/`.** All design artifacts go
   to `.github-gstack-intelligence/state/results/$SLUG/designs/`. This is enforced. See DESIGN_SETUP above.
