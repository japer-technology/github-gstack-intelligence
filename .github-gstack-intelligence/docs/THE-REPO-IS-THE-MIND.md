...
A single GitHub Actions workflow file listens for eight classes of GitHub events: new issues, issue comments, pull requests, pushes to the default branch, manual workflow dispatch, cron schedules, releases, and deployment status changes. A TypeScript router (`router.ts`) inspects each incoming event — its type, any slash-command prefix, attached labels, and the cron schedule that fired — and maps it to one of twenty-six specialized skills. Each skill is a structured prompt that instructs an LLM to perform a specific engineering function: code review, security audit, QA testing, performance benchmarking, root-cause investigation, design audit, developer experience review, retrospective analysis, release documentation, deployment monitoring, planning review, and more.
...
Twenty-six skills cover the full development lifecycle: from brainstorming (`/office-hours`) through planning (`/autoplan`, `/plan-devex-review`), development (`/review`, `/investigate`), design (`/design-consultation`, `/design-shotgun`, `/design-html`, `/design-review`), testing (`/qa`, `/benchmark`, `/devex-review`), security (`/cso`), safety (`/careful`, `/guard`), shipping (`/ship`, `/land-and-deploy`, `/document-release`), deployment verification (`/canary`), and organizational learning (`/retro`, `/learn`, `/health`). As gstack evolves upstream, a refresh workflow pulls the latest skill definitions and commits them — every installation stays current with Tan's latest thinking.
